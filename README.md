...
For layout-only mode, no storefront API URL is required.

```bash
NEXT_PUBLIC_DEFAULT_CHANNEL=default-channel  # Optional channel slug
```
...
npm run generate               # Regenerate GraphQL types (storefront)
npm run generate:checkout      # Regenerate GraphQL types (checkout)
```
...
```env
# Layout-only mode (no Saleor API URL required)
...
SANITY_API_TOKEN=             # Server-only token (never use NEXT_PUBLIC_ prefix)

# Optional: Stripe direct integration
NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY=
STRIPE_SECRET_KEY=            # Server-only secret key
# STRIPE_WEBHOOK_SECRET=whsec_12345

### Vercel: Sanity environment variables
...
Add the following environment variables in your Vercel project settings (Project → Settings → Environment Variables):

- `NEXT_PUBLIC_SANITY_PROJECT_ID` = your Sanity `projectId`
- `NEXT_PUBLIC_SANITY_DATASET` = `production` (or your dataset)

When creating the variables, set the scope for **Production**, **Preview**, and **Development** so all deployments can access them. After adding, trigger a redeploy and disable "Use existing Build Cache" for the first run.

Find where the code expects these vars in [src/lib/sanity/client.ts](src/lib/sanity/client.ts#L1-L40).
```
