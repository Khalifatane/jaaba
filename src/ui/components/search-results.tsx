	...
	return (
		<ul role="list" className="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
			{products.map((product, index) => (
				<li key={product.id}>
					<SearchResultCard product={product} priority={index < 2} />
				</li>
			))}
		</ul>
	);
...
function SearchResultCard({ product, priority }: { product: SearchProduct; priority?: boolean }) {
	...
	const rawDisplay = product.name && product.name.trim() ? product.name : product.slug.replace(/-/g, " ");
	...
	return (
		...
		>
			...
			<div className="p-4">
				...
				<h3 className="font-medium leading-tight text-foreground group-hover:underline">{displayName}</h3>
