...
export function ProductCard({ product, priority = false }: ProductCardProps) {
	...
	// Fallback display name: use `name` if present, otherwise derive from the slug
	...
	const rawDisplay = product.name && product.name.trim() ? product.name : product.slug ? product.slug.replace(/-/g, " ") : "Produit";
	const displayName = toTitleCase(rawDisplay);
		...
		<article className="group">
			<Link href={product.href} className="block">
				...
				<div className="space-y-1.5">
					...
					<h3 className="line-clamp-2 font-medium leading-snug underline-offset-2 md:group-hover:underline text-foreground">
						{displayName}
					</h3>
