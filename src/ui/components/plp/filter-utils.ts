 ...
 *
 * Server-side filters (handled by the product API):
 ...
 * - colors: attribute filtering may require IDs, so we handle it client-side
 * - sizes: Same as colors
 *
 * Note: Server-only helpers can be added alongside this file if needed.
 */
...
/**
 * Build API filter input from URL params.
 */
...
/**
 * Build API order input from sort option.
 */
export function buildSortVariables(sort: SortOption | string | undefined): ProductOrder | undefined {
	...
	const sortMap: Record<string, ProductOrder> = {
		...
		bestselling: { field: "RATING", direction: "DESC" },
	};
 ...
 * Categories and price are filtered server-side when available.
 */
